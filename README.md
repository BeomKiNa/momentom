...
- Typescript 적용을 위해 Webpack 설정 변경
...
1.  Typescript 적용을 위한 Webpack 설정

#### 문제 상황

Typescript를 적용시키기 위해 기존의 Webpack 설정을 변경해야하는 상황

#### 원인

1. TS를 JS로 빌드하기 위해 ts-loader와 babel-loader 중 선택.
2. 현재 JS 파일과 혼용되어있기 때문에 JS파일을 import하면서 발생하는 `Could not find a declaration file for module 'filename'. 'filename' implicitly has an 'any' type` 에러를 해결

#### 해결 방법

1. ts-loader의 경우 tsconfig 파일로 설정을 해줄 수 있지만, ts-loader에 포함된 타입 체크 기능이 빌드 속도 저하의 원인이 되는 듯 하다. 웹팩 문서에서도 기본으로 제공되는 타입 체크 기능은 끄고, 별도의 `ForkTsCheckerWebpackPlugin` 플러그인을 사용해 타입 체크를 사용하길 권장한다.

   babel-loader의 경우 ts-loader에서 제공하지않는 몇 가지 기능을 더 제공한다고 한다.([Why use babel-loader with ts-loader? - stackoverflow](https://stackoverflow.com/questions/49624202/why-use-babel-loader-with-ts-loader)) 또한 기존에는 babel-loder를 사용하더라도 ts-loader가 필요했던 듯 하지만, Babel 7부터는 필요하지 않다고 한다.([How to setup TypeScript + Babel + Webpack? - stackoverflow](https://stackoverflow.com/questions/38320220/how-to-setup-typescript-babel-webpack))

   babel-loader를 사용하기 위해서는 `@babel/preset-typescript`를 사용해야한다. 다만 이 preset만 사용할 경우 ES6로 빌드되기 때문에 `@babel/preset-env`를 사용해야 ES5 이전의 JS로 빌드된다.

   마지막으로 `@babel/preset-typescript`는 `@babel/plugin-transform-typescript` 플러그인을 기본으로 포함하지만, 이 역시 빌드 때 타입 체크를 하지 않는다. 따라서 ts-loader와 마찬가지로 `ForkTsCheckerWebpackPlugin` 플러그인을 사용해야 타입을 체크할 수 있다.

2. TS파일에 JS파일을 import하면 `Could not find a declaration file for module 'filename'. 'filename' implicitly has an 'any' type` 에러가 발생하는데, 이를 해결하기 위해선 tsconfig.json 파일에서 `"noImplicitAny": false`로 지정해주어야한다.

#### 참고

- [TypeScript - Webpack](https://webpack.js.org/guides/typescript/#root)
- [Build Performance - Webpack](https://webpack.js.org/guides/build-performance/)
- [Why use babel-loader with ts-loader? - stackoverflow](https://stackoverflow.com/questions/49624202/why-use-babel-loader-with-ts-loader))
- [How to setup TypeScript + Babel + Webpack? - stackoverflow](https://stackoverflow.com/questions/38320220/how-to-setup-typescript-babel-webpack)
- [How to fix error TS7016: Could not find a declaration file for module ‘XYZ’. ‘file.js’ implicitly has an ‘any’ type](https://pjausovec.medium.com/how-to-fix-error-ts7016-could-not-find-a-declaration-file-for-module-xyz-has-an-any-type-ecab588800a8)
- [How to resolve regeneratorRuntime is not defined](https://bbon.kr/how-to-resolve-regeneratorruntime-is-not-defined/)
...
- [[Typescript & React & Webpack 환경설정 3편] Webpack 설정을 해보자](https://koras02.tistory.com/42?category=967574)
